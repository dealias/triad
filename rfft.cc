		   ...
		   int bitreverse)
{
	...
	mfft(data,log2nx,isign,ny,ny,1,bitreverse);
	mfft(data,log2ny,isign,nx,1,ny,bitreverse);
}
			  ...
			  int isign, int bitreverse)
{
	...
	mrcfft(data,log2ny,isign,nx,nx1,1,bitreverse);
...
#pragma ivdep
	...
	mfft(data,log2nx,isign,nyp,1,nx1,bitreverse);
}
...
// condition (see crfft2dT_sym below).
// The origin of the Fourier domain is located at (nx/2,0).
//           log2nx contains the base-2 logarithm of nx.
//           log2ny contains the base-2 logarithm of ny.
//           isign is +1 for a forward transform, -1 for an inverse transform.
...
#pragma ivdep
	...
	mcrfft(data,log2ny,isign,nx,nx1,1,bitreverse);
}
...
#pragma ivdep
	...
	crfft2dT(data,log2nx,log2ny,isign,bitreverse);
}
			 ...
			 int isign, int bitreverse)
{
	...
	mrcfft(data,log2ny,isign,nx,1,nyp,bitreverse);
...
#pragma ivdep
	...
	mfft(data,log2nx,isign,nyp,nyp,1,bitreverse);
}
...
// each i=0,...,nx-1. 
// The values corresponding to ky=0 are assumed to satisfy the reality
// condition (see crfft2d_sym below).
// The origin of the Fourier domain is located at (nx/2,0).
//           log2nx contains the base-2 logarithm of nx.
//           log2ny contains the base-2 logarithm of ny.
//           isign is +1 for a forward transform, -1 for an inverse transform.
...
#pragma ivdep
	...
	mcrfft(data,log2ny,isign,nx,1,nyp,bitreverse);
}
...
#pragma ivdep
	...
	crfft2d(data,log2nx,log2ny,isign,bitreverse);
}
...
void fft3d(Complex *data, unsigned int log2nx, unsigned int log2ny,
		   unsigned int log2nz, int isign, int bitreverse)
{
	unsigned int nx=1 << log2nx;
	unsigned int ny=1 << log2ny;
	...
	mfft(data,log2nx,isign,nyz,nyz,1,bitreverse);
	for(unsigned int i=0; i < nx; i++)
		mfft(data+i*nyz,log2ny,isign,nz,nz,1,bitreverse);
	mfft(data,log2nz,isign,nx*ny,1,nz,bitreverse);
}
			 ...
			 unsigned int log2nz, int isign, int bitreverse)
{
	...
	mrcfft(data,log2nz,isign,nx*ny,1,nzp,bitreverse);
...
#pragma ivdep
	...
	mfft(data,log2nx,isign,nyzp,nyzp,1,bitreverse);
	
	for(i=0; i < nx; i++)
		mfft(data+i*nyzp,log2ny,isign,nzp,nzp,1,bitreverse);
}
...
// condition (see crfft3d_sym below).
// The origin of the Fourier domain is located at (nx/2,ny/2,0).
//           log2nx contains the base-2 logarithm of nx.
//           log2ny contains the base-2 logarithm of ny.
//           log2nz contains the base-2 logarithm of nz.
...
#pragma ivdep
	...
	mcrfft(data,log2nz,isign,nx*ny,1,nzp,bitreverse);
}
...
#pragma ivdep
	...
	crfft3d(data,log2nx,log2ny,log2nz,isign,bitreverse);
}
