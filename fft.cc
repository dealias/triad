...
static void fft_br(Complex *data, unsigned int log2n, int isign,
				   Real scale=1.0);
static void mfft_br(Complex *data, unsigned int log2n, int isign,
...
void fft(Complex *data, unsigned int log2n, int isign, Real scale,
		 int bitreverse) 
{
	if(bitreverse == 1) {fft_br(data,log2n,isign,scale); return;}
...
// Return the Fourier transforms of nk Complex vectors.
		  ...
		  unsigned int inc1, unsigned int inc2, Real scale, int bitreverse)
{
	...
	if(bitreverse == 1) {mfft_br(data,log2n,isign,nk,inc1,inc2,scale); return;}
...
// Return the Fourier transform of a bit-reversed Complex vector.
// Before calling, data must be allocated as Complex[n].
// On entry: data contains the n Complex values;
//           log2n contains the base-2 logarithm of n;
//           isign is the sign (+/- 1) of the phase;
...
static void fft_br(Complex *data, unsigned int log2n, int isign, Real scale)
{
	if(log2n > wpTableSize) fft_init(log2n);
...
// Return the Fourier transforms of nk bit-reversed Complex vectors.
// Before calling, data must be allocated as Complex[nk*n].
// On entry: data contains the n Complex values for each k=0,...,nk-1;
//           log2n contains the base-2 logarithm of n;
//           isign is the sign (+/- 1) of the phase;
